...
# Problem 3: This python script solves the active contraction of a left ventricular model.

# The undeformed geometry is a beam of length 10 mm, and cross-section of 1 by 1 mm square. The constitutive model used
# is the Guccione transversely isotropic with strain energy function:
...
# Identify basal and endocardial nodes using mathematical formulation of the truncated ellipsoid model.Intialise
# geometric field also.
basal_nodes = []
endocardial_nodes = []
...
GeometricFieldExport(region,
                     "LVContraction_trilinear_undeformed_" + str(elems[2]) + "-" + str(elems[1]) + "-" + str(elems[0]))
...
# Set up the independent field which contains the active contraction
#contraction = 60 #kPa
[independentField, equationsSet] = IndependentFieldSetUp(independentFieldUserNumber, equationsSet)

# Set up equations set
EquationsSetSetUp(equationsSet)

deformedField = SetUpDeformedField(deformedFieldUserNumber, decomposition, region, option)
...
for i in range(0, len(p_increm)):
    ...
    p = p + incr
    Ta = Ta + Ta_increm[i]
    IndependentFieldInitialise(independentField, Ta)
    print 'Applying pressure increment of: ', incr
    print 'Applying Ta increment of: ', Ta_increm[i]
    print 'Current pressure is: ', p
    print 'Current Ta is ', Ta
    [problem, solverEquations] = ProblemSolverSetup(equationsSet, problemUserNumber, 1, 1e-6, cellMLOption)
    BCEndoPressure(solverEquations, dependentField, endocardial_nodes, incr, basal_nodes, option)
...
independentField.Destroy()

# Export solution
filename = "LVContraction_trilinear_" + str(elems[2]) + "-" + str(elems[1]) + "-" + str(elems[0])
ExportResults(dependentField, deformedField, region, filename)

# Evaluate displacement
deformed = exfile.Exnode(
    "LVContraction_trilinear_" + str(elems[2]) + "-" + str(elems[1]) + "-" + str(elems[0]) + ".part0.exnode")
...
mse = math.sqrt(np.linalg.norm(disp))
# Write mse to a file
with open('rmse.txt', 'a') as f:
    f.write(str(elems[2]) + " " + str(elems[1]) + " " + str(elems[0]) + " " + str(mse) + "\n")
    f.close()
...
with open("displacement" + str(elems[2]) + "-" + str(elems[1]) + "-" + str(elems[0]) + ".exdata", 'w') as fid:
    ...
    for i in range(0, len(disp)):
        ...
        fid.write(' ' + str(all_nodes[idx - 1][0]) + ' ' + str(all_nodes[idx - 1][1]) + ' ' + str(
            all_nodes[idx - 1][2]) + '\n')
